  ...
  return (
    <style>{`
      ...
      .leaflet-top.leaflet-left .leaflet-control-zoom {
        z-index: 2001; box-shadow: 0 10px 24px rgba(0,0,0,.2);
      ...
      .hide-btn { padding:8px 12px; border-radius:12px; border:1px solid var(--border); background:transparent;
        cursor:pointer; color:var(--text); transition:transform .18s ease; }
...
export default function App() {
  ...
  const geoRef = useRef(null);       // GeoJSON layer
  const highlightRef = useRef(null); // animated outline layer
  ...
  // Polygons style
  const polygonStyle = () => ({
    color: getComputedStyle(document.documentElement).getPropertyValue("--brand1").trim(),
    weight: 0.7,
    fillColor: getComputedStyle(document.documentElement).getPropertyValue("--brand2").trim(),
    fillOpacity: 0.12,
  });
  ...
  // Get a live Leaflet map instance (ref or from the GeoJSON layer)
  function getMap() {
    return mapRef.current || geoRef.current?._map || null;
  }

  // Make sure the high-z outline pane exists on the given map
  function ensureOutlinePane(map) {
    if (!map) return;
    let pane = map.getPane("outline-pane");
    if (!pane) pane = map.createPane("outline-pane");
    pane.classList.add("outline-pane"); // picks CSS you already have
  }


  // RELIABLE OUTLINE with static-first, then animated class + fallback rect
  function outlineLocality(name, retry = 0) {
    const map = getMap();
    const g = geoRef.current;
    if (!g || !name) return;

    // If map still not ready, retry a few times (React StrictMode / slow mounts)
    if (!map) {
      if (retry < 10) {
        setTimeout(() => outlineLocality(name, retry + 1), 60);
      } else {
        console.warn("[outline] map not ready after retries");
      }
      return;
    }
    ...
    // Fallback: dashed bounds rectangle if no matching features
    if (!feats.length) {
      ...
      if (entry?.bounds?.isValid()) {
        ...
        console.warn("[outline] no features and no bounds for", name);
      }
      return;
    }

    // Build visible static outline first
    const hl = L.geoJSON(
    ...
    // Next frame, apply animated class to paths
    const applyAnim = () => {
      hl.eachLayer((lyr) => {
  ...
  return (
    <div className="map-wrap">
      ...
      {collapsed && (
        <button className="launcher" onClick={() => setCollapsed(false)} title="Show localities">
      ...
      {!collapsed && (
        <div className="panel">
          <div className="controls">
            ...
            <button className="hide-btn" onClick={() => setCollapsed(true)}>Hide</button>
          </div>
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    if (map && b && b.isValid()) {
                      ...
                      setPendingBounds(b);
                      outlineLocality(name); // will retry until map is ready
                    }
                  }}
      ...
      >
        ...
        {pendingBounds && <FlyTo bounds={pendingBounds} />}
        ...
        {shapeData && (
          <GeoJSON
            ...
            renderer={L.svg()}   // keep SVG so outline CSS can apply
          />
        )}
      </MapContainer>
